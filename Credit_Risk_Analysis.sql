...
SELECT cdt.[CUSTID]
   ...
   /*,case when ((Credit_amount)/Duration_in_months) <= (0.333 * (coalesce(lodgement-withdrawal,0)/6)) and coalesce(Average_salary,0) = 0 then 170
			when ((Credit_amount)/Duration_in_months) > (0.333 * (coalesce(lodgement-withdrawal,0)/6)) 
			and ((Credit_amount)/Duration_in_months) <= (0.50 * (coalesce(lodgement-withdrawal,0)/6)) and coalesce(Average_salary,0) =0 then 85
			when ((Credit_amount)/Duration_in_months) > (0.50 * (coalesce(lodgement-withdrawal,0)/6)) and coalesce(Average_salary,0) = 0 then 10
			when ((Credit_amount)/Duration_in_months) <= (0.333 * (coalesce(Average_salary,0))) then 170
			when ((Credit_amount)/Duration_in_months) > (0.333 * (coalesce(Average_salary,0))) 
			and ((Credit_amount)/Duration_in_months) <= (0.50 *(coalesce(Average_salary,0))) then 85
			when ((Credit_amount)/Duration_in_months) > (0.50 * (coalesce(Average_salary,0))) then 10  
			end credit_utilization_sc */
	...
	,[Credit_amount]
	,[Duration_in_months]
	,(select ((case when datediff(year,[DOB], getdate()) <= 17 then 0
			...
			(case when No_of_accounts < 2 then 5 else 9 end) +
			...
			(case when ((Credit_amount )/Duration_in_months) < (0.33 * (coalesce(lodgement-withdrawal,0)/6)) and coalesce(Average_salary,0) = 0 then 170
				when ((Credit_amount )/Duration_in_months) > (0.33 * (coalesce(lodgement-withdrawal,0)/6)) 
				and ((Credit_amount )/Duration_in_months) < (0.50 * (coalesce(lodgement-withdrawal,0)/6)) and coalesce(Average_salary,0) =0 then 85
				when ((Credit_amount )/Duration_in_months) > (0.50 * (coalesce(lodgement-withdrawal,0)/6)) and coalesce(Average_salary,0) = 0 then 10
				when ((Credit_amount )/Duration_in_months) < (0.33 * (coalesce(Average_salary,0))) then 170
...
FROM [DEMO].[dbo].[Demo_credit_data_dup] cdt
--join with the Demography Table
left outer join DEMO_Demography bio
 on bio.CUSTID = cdt.CUSTID 
--join and compute total transaction for the period from the transaction table
left outer join (select custid, isnull(sum(txn.txn_amount),0) lodgement, isnull(count(txn.txn_amount), 0)lodgement_freq
...
left outer join (select custid, isnull(sum(txn.txn_amount),0) withdrawal, isnull(count(txn.txn_amount), 0) withdrawal_freq
                from [dbo].[Demo_Transactions_dup] txn
				where (FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -6,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -5,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -4,getdate()), 'MM-yyyy')
...
--join and compute the Average salary for the period from the transaction table
...
--join and compute Custommer lifestyle spending for the period from the transaction table
left outer join (select custid, isnull(sum(Txn_amount),0) utility_spending_amount ,isnull(count(txn_amount),0) utility_spending_freq
                from [dbo].[Demo_Transactions_dup] txn
				where (FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -6,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -5,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -4,getdate()), 'MM-yyyy')
				...
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -2,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -1,getdate()), 'MM-yyyy'))
                and txn.[Txn_Amount] is not null and txn.Txn_type =  'D'  and (Txn_Narration = 'Electricity bill' 
				or Txn_Narration = 'Dstv bill' or Txn_Narration = 'waste disposal bill')
				group by custid, Txn_Type) ut 
on cdt.CUSTID = ut.CUSTID
left outer join (select custid, isnull(sum(Txn_amount),0) Transfers_amount ,isnull(count(txn_amount),0) Transfers_freq
                from [dbo].[Demo_Transactions_dup] txn
				where (FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -6,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -5,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -4,getdate()), 'MM-yyyy')
				...
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -2,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -1,getdate()), 'MM-yyyy'))
                and txn.[Txn_Amount] is not null and txn.Txn_type =  'D'  and (Txn_Narration = 'Transfer btw Customers' 
				or Txn_Narration = 'International transfers')
				group by custid, Txn_Type) tr
on cdt.CUSTID = tr.CUSTID
left outer join (select custid, isnull(sum(Txn_amount),0) fees_amount ,isnull(count(txn_amount),0) fees_freq
                from [dbo].[Demo_Transactions_dup] txn
				where (FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -6,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -5,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -4,getdate()), 'MM-yyyy')
				...
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -2,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -1,getdate()), 'MM-yyyy'))
                and txn.[Txn_Amount] is not null and txn.Txn_type =  'D'  and (Txn_Narration = 'School fees' 
				or Txn_Narration = 'Rent')
				group by custid, Txn_Type) fe
on cdt.CUSTID = fe.CUSTID
left outer join (select custid, isnull(sum(Txn_amount),0)Purchase_amount ,isnull(count(txn_amount),0) purchase_freq
                from [dbo].[Demo_Transactions_dup] txn
				where (FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -6,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -5,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -4,getdate()), 'MM-yyyy')
				...
				or Txn_Narration = 'Online Purchases' or txn_narration = 'online shopping' or Txn_Narration = 'Betting sites')
				group by custid, Txn_Type) pu 
on cdt.CUSTID = pu.CUSTID
left outer join (select custid, isnull(sum(Txn_amount),0) mobile_nw_amount ,isnull(count(txn_amount),0) mobile_nw_freq
                from [dbo].[Demo_Transactions_dup] txn
				where (FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -6,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -5,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -4,getdate()), 'MM-yyyy')
				...
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -2,getdate()), 'MM-yyyy')
				or FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -1,getdate()), 'MM-yyyy'))
                and txn.[Txn_Amount] is not null and txn.Txn_type =  'D'  and (Txn_Narration = 'data subscription' 
				or Txn_Narration = 'airtime purchase')
				group by custid, Txn_Type) mn
on cdt.CUSTID = mn.CUSTID
--join and compute Total Credit transaction
left outer join (select custid, isnull(sum(txn.txn_amount),0) lodgement_all
                from [dbo].[Demo_Transactions_dup] txn
...
left outer join (select custid, isnull(sum(txn.txn_amount),0) withdrawal_all
                from [dbo].[Demo_Transactions_dup] txn
				-- where FORMAT (txn.[TxnDate], 'MM-yyyy') = FORMAT (dateadd(month, -5,getdate()), 'MM-yyyy')
